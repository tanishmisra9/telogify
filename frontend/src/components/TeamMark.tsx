 ...
 * Weight/color/width come from the caller via className. */
export function TeamMark({ team, className }: { team: string | null | undefined; className?: string }) {
  return (
    <span className={`inline-flex min-w-0 items-center gap-2 ${className ?? ''}`}>
      <TeamRule team={team} />
      <span className="truncate">{team ?? 'Unknown'}</span>
    </span>
  )
}
